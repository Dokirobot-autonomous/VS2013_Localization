...
//	//画像読み込み
//	std::string filename;
//	//filename = "E://Data/Localization/Measurement/" + MEASUREMENT_DATE + "/" + MEASUREMENT_TIME + "/img/img_no" + std::to_string(1) + "_" + std::to_string(1) + "th.bmp";
//	//cv::Mat colorImg1 = cv::imread(filename);
//	cv::Mat colorImg1 = cv::imread("../input/img_no1_98th.bmp");
//	filename = "E://Data/Localization/Measurement/" + MEASUREMENT_DATE + "/" + MEASUREMENT_TIME + "/img/img_no" + std::to_string(1) + "_" + std::to_string(5) + "th.bmp";
//	//cv::Mat colorImg2 = cv::imread(filename);
//	cv::Mat colorImg2 = cv::imread("../input/img_no1_170th.bmp");
...
//	{
//		/*  Operatorの塗りつぶし  */
//		cv::Point center(colorImg1.cols / 2.1, colorImg1.rows / 2);
...
//	//SURF
//	cv::SiftFeatureDetector detector1(1000);
//	cv::SiftDescriptorExtractor extractor1;
//	cv::SiftFeatureDetector detector2(1000);
//	cv::SiftDescriptorExtractor extractor2;
//
//	//画像から特徴点を検出
//	std::vector<cv::KeyPoint> keypoints1;
//	detector1.detect(grayImg1, keypoints1);
//	std::vector<cv::KeyPoint> keypoints2;
//	detector2.detect(grayImg2, keypoints2);
//
//	// Operatorとロボット前方の棒部分の特徴量を排除
//	int x_interval = grayImg1.cols/12.0;
//	int y_interval = 100;
//	std::vector<cv::KeyPoint> keypoints1_1, keypoints2_2;
//	for (auto& key : keypoints1){
//		if (key.pt.x < grayImg1.cols / 2.0 - x_interval || key.pt.x > grayImg1.cols / 2.0 + x_interval){
//			if (key.pt.y < grayImg1.rows / 2.0 - y_interval || key.pt.y > grayImg1.rows / 2.0){
//				keypoints1_1.push_back(key);
//			}
//		}
//	}
//	for (auto& key : keypoints2){
//		if (key.pt.x < grayImg2.cols / 2.0 - x_interval || key.pt.x > grayImg2.cols / 2.0 + x_interval){
//			if (key.pt.y < grayImg2.rows / 2.0 - y_interval || key.pt.y > grayImg2.rows / 2.0){
//				keypoints2_2.push_back(key);
//			}
//		}
//	}
//
//	//画像の特徴点における特徴量を抽出
//	cv::Mat descriptors1;
//	extractor1.compute(grayImg1, keypoints1_1, descriptors1);
//	cv::Mat descriptors2;
//	extractor2.compute(grayImg2, keypoints2_2, descriptors2);
//
//	while (true){
//
//		//特徴点の対応付け
...
//		//対応づけされた画像の用意
//		cv::Mat matchedImg;
//		cv::drawMatches(colorImg1, keypoints1_1, colorImg2, keypoints2_2, matches, matchedImg);
//
